...
function Login() {
  ...
  function submit() {
    ...
    playersList.map((element) => {
      if (
        element.username === state.player.username &&
        element.password === state.player.password
      ) {
  ...
  return (
    <div className="login">
      <section>
        <h1>Login</h1>
        <form>
          <InputBox placeholder={"username"} callBackChange={getUsername} />
          <InputBox
            type={"password"}
          ...
          <Button callBackClick={submit} label={"login"} classCss={"primary"} />
        </form>
